...
export function ProductCard({ product }: ProductCardProps) {
  ...
  const description =
    ...
    (product as any).description?.en ??
    (product as any).description ??
    ""

  const colors: string[] = Array.isArray((product as any).colors) ? (product as any).colors : []
  const sizes: string[] = Array.isArray((product as any).sizes) ? (product as any).sizes : []
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-square overflow-hidden bg-muted">
        ...
        <img
          src={(product as any).image || "/placeholder.svg"}
          alt={name}
        ...
        <div
          className={`absolute inset-0 bg-gradient-to-t from-background/95 via-background/50 to-transparent transition-all duration-500 ${
        ...
        >
          <div className="absolute bottom-4 left-4 right-4 flex flex-col gap-2 sm:flex-row transform transition-all duration-500">
            <Button
              asChild
              size="sm"
              className="flex-1 hover:scale-105 transition-transform duration-300 relative overflow-hidden group/btn"
            >
      ...
      <CardFooter className="p-4 pt-0 flex items-center justify-between gap-4">
        <div className="group-hover:scale-105 transition-transform duration-300">
          ...
          <p className="text-2xl font-bold text-primary bg-gradient-to-r from-primary to-accent bg-clip-text group-hover:text-transparent transition-all duration-300">
            {formatCurrency((product as any).basePrice ?? 0, locale)}
          </p>
        </div>
